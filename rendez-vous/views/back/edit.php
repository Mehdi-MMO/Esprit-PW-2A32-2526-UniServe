...
<head>
    ...
    <style>
        ...
        .btn { background: #2196F3; color: white; padding: 10px 20px; border: none; cursor: pointer; border-radius: 4px; }
...
<body>
    ...
    <form method="POST" action="index.php?page=back&action=update" onsubmit="return validerFormulaire()">
        ...
        <label>Titre :</label>
        <input type="text" name="titre" id="titre" value="<?= htmlspecialchars($_POST['titre'] ?? $this->model->titre) ?>">
        <span class="error" id="err_titre"><?= $errors['titre'] ?? '' ?></span>

        <label>Date :</label>
        <input type="text" name="date_rdv" id="date_rdv" placeholder="YYYY-MM-DD" value="<?= $_POST['date_rdv'] ?? $this->model->date_rdv ?>">
        <span class="error" id="err_date"><?= $errors['date_rdv'] ?? '' ?></span>

        <label>Heure :</label>
        <input type="text" name="heure_rdv" id="heure_rdv" placeholder="HH:MM" value="<?= $_POST['heure_rdv'] ?? $this->model->heure_rdv ?>">
        <span class="error" id="err_heure"><?= $errors['heure_rdv'] ?? '' ?></span>

        <label>Description :</label>
        <textarea name="description" rows="4"><?= htmlspecialchars($_POST['description'] ?? $this->model->description) ?></textarea>
