...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .btn-submit { background: #26a69a; color: white; padding: 12px 30px; border: none; border-radius: 8px; font-size: 15px; cursor: pointer; }
...
<body>
    ...
    <div class="main">
        <div class="header">
            ...
            <p>Modifiez les informations du rendez-vous sélectionné.</p>
        </div>

        <div class="form-card">
            <h2>Modifier Rendez-vous #<?= $this->model->id ?></h2>
            <p>Tous les champs marqués sont obligatoires.</p>

            <form method="POST" action="index.php?page=back&action=update" onsubmit="return validerFormulaire()">
                <input type="hidden" name="id" value="<?= $this->model->id ?>">

                <div class="form-group">
                    <label>Titre</label>
                    <input type="text" name="titre" id="titre" placeholder="Ex: Consultation médecin"
                        value="<?= htmlspecialchars($_POST['titre'] ?? $this->model->titre) ?>">
                    <span class="error" id="err_titre"><?= $errors['titre'] ?? '' ?></span>
                </div>

                <div class="form-group">
                    <label>Date</label>
                    <input type="text" name="date_rdv" id="date_rdv" placeholder="YYYY-MM-DD"
                        value="<?= $_POST['date_rdv'] ?? $this->model->date_rdv ?>">
                    <span class="error" id="err_date"><?= $errors['date_rdv'] ?? '' ?></span>
                </div>

                <div class="form-group">
                    <label>Heure</label>
                    <input type="text" name="heure_rdv" id="heure_rdv" placeholder="HH:MM"
                        value="<?= $_POST['heure_rdv'] ?? $this->model->heure_rdv ?>">
                    <span class="error" id="err_heure"><?= $errors['heure_rdv'] ?? '' ?></span>
                </div>

                <div class="form-group">
                    <label>Description</label>
                    <textarea name="description" rows="4" placeholder="Décrivez le rendez-vous..."><?= htmlspecialchars($_POST['description'] ?? $this->model->description) ?></textarea>
                </div>
