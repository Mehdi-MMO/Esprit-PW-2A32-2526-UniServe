...
$statCards = [
    ...
    ['title' => 'Confirm├⌐s', 'value' => $ds['confirme'] ?? 0, 'color' => 'success', 'icon' => 'fa-solid fa-circle-check'],
    ['title' => 'Annul├⌐s', 'value' => $ds['annule'] ?? 0, 'color' => 'danger', 'icon' => 'fa-solid fa-ban'],
];
...
<div class="d-flex align-items-center justify-content-between flex-wrap gap-3 us-page-header mb-4">
    <div>
        ...
        <p class="text-muted mb-0">G├⌐rez les rendez-vous et les bureaux depuis cette page.</p>
    </div>
</div>
...
<?php if ($tab === 'rdv'): ?>
        ...
        <div class="card-body p-3">
            <form method="get" action="<?= $this->url('/rendezvous') ?>" class="row g-2 align-items-end">
                ...
                <div class="col-lg-4">
                    ...
                    <input type="text" class="form-control" id="rq" name="q" value="<?= htmlspecialchars($q, ENT_QUOTES, 'UTF-8') ?>" placeholder="Rechercher par nom, sujet ou bureauΓÇª">
                </div>
                <div class="col-md-2">
                ...
                <div class="col-md-2">
                    ...
                    <select class="form-select" id="srt" name="sort">
                        <option value="date_desc" <?= $sort === 'date_desc' ? 'selected' : '' ?>>Date r├⌐cente</option>
                ...
                <div class="col-lg-4 d-flex flex-wrap gap-2 justify-content-lg-end">
                    ...
                    <a href="<?= $this->url('/rendezvous') ?>?tab=rdv" class="btn btn-outline-secondary">R├⌐initialiser</a>
                        ...
                        <?php else: ?>
                            <?php foreach ($rdvs as $row): ?>
                                ...
                                <tr>
                                    ...
                                    <td><?= htmlspecialchars((string) ($row['motif'] ?? 'ΓÇö'), ENT_QUOTES, 'UTF-8') ?></td>
                                    <td>
                                        <div class="d-flex align-items-start gap-2 text-nowrap">
                                            ...
                                            <div class="small">
                                                ...
                                                <div class="text-muted">ΓåÆ <?= htmlspecialchars($fmtRdv($d2), ENT_QUOTES, 'UTF-8') ?></div>
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                    ...
                                    <td class="text-end">
                                        <div class="d-flex flex-column align-items-end gap-2">
                                            ...
                                            <form method="post" action="<?= $this->url('/rendezvous/adminDelete/' . $sid) ?>" onsubmit="return confirm('Supprimer d├⌐finitivement ce rendez-vous ?');">
        ...
        <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mt-3">
            ...
            <nav aria-label="Pagination">
                <ul class="pagination pagination-sm mb-0">
                    <?php if ($pg > 1): ?>
                        <li class="page-item">
                            <a class="page-link" href="<?= htmlspecialchars($this->url('/rendezvous') . '?' . $buildRdvQuery(['page' => $pg - 1]), ENT_QUOTES, 'UTF-8') ?>">Pr├⌐c.</a>
                        </li>
                    <?php endif; ?>
                    <?php for ($i = 1; $i <= $tp; $i++): ?>
    ...
    <?php if (($ds['termine'] ?? 0) > 0): ?>
        <p class="text-muted small mt-3 mb-0"><strong>Termin├⌐s :</strong> <?= (int) ($ds['termine'] ?? 0) ?> (inclus dans le total ci-dessus)</p>
    <?php endif; ?>
        ...
        <div class="card-body p-3">
            <form method="get" action="<?= $this->url('/rendezvous') ?>" class="row g-2 align-items-end">
                ...
                <div class="col-lg-6">
                    ...
                    <input type="text" class="form-control" id="bbq" name="bq" value="<?= htmlspecialchars($bureau_bq, ENT_QUOTES, 'UTF-8') ?>" placeholder="Nom, localisation ou type de serviceΓÇª">
                </div>
                <div class="col-lg-6 d-flex flex-wrap gap-2 justify-content-lg-end">
                    <button type="submit" class="btn btn-primary"><i class="fa-solid fa-magnifying-glass me-2"></i>Rechercher</button>
                    <a href="<?= $this->url('/rendezvous') ?>?tab=bureaux" class="btn btn-outline-secondary">R├⌐initialiser</a>
    ...
    <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
        <p class="text-muted small mb-0">
            <?php if ($nb_bureaux_filtered > 0): ?>
                Affichage <?= $bureau_from ?>-<?= $bureau_to ?> sur <?= $nb_bureaux_filtered ?> bureau(x)<?= $bureau_bq !== '' ? ' (filtr├⌐s)' : '' ?>
                    ...
                    <div class="card h-100 us-bureau-card shadow-sm border-0">
                        <div class="card-body d-flex flex-column">
                            ...
                            <p class="small text-muted mb-2 d-flex gap-2">
                                ...
                                <span><?= htmlspecialchars(trim((string) ($b['localisation'] ?? '')) !== '' ? (string) $b['localisation'] : 'ΓÇö', ENT_QUOTES, 'UTF-8') ?></span>
                            </p>
            ...
            <nav aria-label="Pagination bureaux">
                <ul class="pagination pagination-sm mb-0">
                    <?php if ($bureau_page > 1): ?>
                        <li class="page-item">
                            <a class="page-link" href="<?= htmlspecialchars($this->url('/rendezvous') . '?' . $buildBureauQuery(['bpage' => $bureau_page - 1]), ENT_QUOTES, 'UTF-8') ?>">Pr├⌐c.</a>
                        </li>
                    <?php endif; ?>
