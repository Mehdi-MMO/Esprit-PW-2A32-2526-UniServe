...
<head>
    ...
    <meta name="description" content="Portail institutionnel UniServe : demandes de service, rendez-vous, documents, certifications, clubs, événements et notifications.">
...
<body class="us-landing">
    ...
    <nav class="navbar navbar-expand-lg us-topbar us-landing-nav navbar-dark shadow-sm" aria-label="Navigation principale">
        <div class="container">
            <a class="navbar-brand d-inline-flex align-items-center us-navbar-brand-logo" href="<?= $this->url('/') ?>" aria-label="UniServe — accueil">
                <?= us_brand_logo_html($this, 'us-brand-logo--nav us-brand-logo--on-dark', false) ?>
            </a>
            <button class="navbar-toggler border-0" type="button" data-bs-toggle="collapse" data-bs-target="#landingNav"
                    aria-controls="landingNav" aria-expanded="false" aria-label="Ouvrir le menu">
                <span class="navbar-toggler-icon"></span>
            </button>
            <div class="collapse navbar-collapse" id="landingNav">
                <ul class="navbar-nav ms-auto me-3 gap-1">
                    <li class="nav-item">
                        <a class="nav-link fw-600" href="#fonctionnalites">Fonctionnalités</a>
                    </li>
                </ul>
                <div class="d-flex align-items-center gap-2">
                    <a href="<?= $this->url('/auth/login') ?>" class="btn btn-outline-light btn-sm px-3 fw-600">
                        Se connecter
                    </a>
                </div>
            </div>
    ...
    <footer class="us-landing-footer py-4 border-top" role="contentinfo">
        <div class="container">
            <div class="d-flex flex-column flex-md-row align-items-center justify-content-between gap-3 small text-muted">
                <span>&copy; <?= date('Y') ?> UniServe. Tous droits réservés.</span>
                <div class="d-flex gap-3">
                    <a href="<?= $this->url('/auth/login') ?>" class="text-muted text-decoration-none landing-footer-link">Connexion</a>
                </div>
            </div>
        </div>
    </footer>
