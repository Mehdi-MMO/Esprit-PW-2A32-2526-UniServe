document.addEventListener('DOMContentLoaded', function () {
    ...
    if (chatRoot) {
        ...
        const endpoint = chatRoot.getAttribute('data-ai-endpoint') || 'index.php?url=chatbot/ask';
        const history = [];
    ...
    const ensureMap = function () {
        ...
        map.on('click', function (e) {
            ...
            if (pickerTargetInput) {
                ...
                setStatus('Point selectionne: ' + lat + ', ' + lng);
            }
        });
    };
    ...
    const geocode = async function () {
        ...
        try {
            ...
            setStatus(first.display_name || 'Adresse trouvee.');
            return point;
        } catch (_err) {
    ...
    const drawRoute = async function (fromPoint, toPoint) {
        ...
        setStatus('Itineraire trace (bleu): ' + km.toFixed(1) + ' km, environ ' + eta + '.');
    };
    ...
    const startRouteFlow = async function (address) {
        ...
        const query = (address || '').trim();
        if (query === '') {
            setStatus('Lieu evenement manquant.');
            return;
        }

        searchInput.value = query;
        setStatus('Recherche du lieu evenement...');
        const destinationPoint = await geocode();
        if (!destinationPoint) {
            setStatus('Lieu evenement introuvable.');
            return;
        }
        ...
        setStatus('Autorisez "Ma position" pour tracer votre itineraire.');
    };
    ...
    locateBtn.addEventListener('click', function () {
        ...
        navigator.geolocation.getCurrentPosition(function (pos) {
            ...
            if (activeDestinationPoint !== null) {
                ...
                setStatus('Position actuelle detectee.');
            }
        }, function () {
            setStatus('Impossible de recuperer votre position.');
    ...
    document.querySelectorAll('[data-map-route-btn]').forEach(function (btn) {
        btn.addEventListener('click', function () {
            ...
            startRouteFlow(address).catch(function () {
                setStatus('Impossible de lancer l itineraire.');
            });
        });
    });
