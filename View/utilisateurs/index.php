...
<div class="card border-0 shadow-sm">
    <div class="card-body p-3 p-md-4">
        ...
        <form id="utilisateurs-filters" method="get" action="<?= $this->url('/utilisateurs') ?>" class="row g-2 align-items-end mb-3">
            <div class="col-lg-4">
            ...
            <div class="col-lg-2 d-flex gap-2">
                ...
                <a href="<?= $this->url('/utilisateurs') ?>" class="btn btn-outline-secondary w-100">Reset</a>
            </div>
        </form>

        <?php if (empty($users)): ?>
            <div class="alert alert-info mb-0" role="alert">Aucun utilisateur trouvé.</div>
        <?php else: ?>
                ...
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody id="users-rows">
                        <?php foreach ($users as $u): ?>
                            <tr>
                                <td class="text-muted"><?= htmlspecialchars((string) ($u['id'] ?? ''), ENT_QUOTES, 'UTF-8') ?></td>
                                <td>
                                    <div class="fw-semibold">
                                        <?= htmlspecialchars((string) ($u['prenom'] ?? ''), ENT_QUOTES, 'UTF-8') ?>
                                        <?= htmlspecialchars((string) ($u['nom'] ?? ''), ENT_QUOTES, 'UTF-8') ?>
                                    </div>
                                    <div class="text-muted small"><?= htmlspecialchars((string) ($u['departement'] ?? ''), ENT_QUOTES, 'UTF-8') ?></div>
                                </td>
                                ...
                                <td class="text-end">
                                    ...
                                    <?php if ($isSingleAdmin): ?>
                                        <button class="btn btn-outline-danger btn-sm ms-2" type="button" disabled title="Suppression bloquée (admin unique)">
