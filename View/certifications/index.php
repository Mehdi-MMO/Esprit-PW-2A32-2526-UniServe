<div class="container-fluid py-4 min-vh-100">
    ...
    <div class="px-3">
        ...
        <div class="card shadow mb-4">
            <div class="card-header border-bottom">
                <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
                    <h5 class="m-0"><i class="bi bi-journals me-2 text-info"></i>Liste des Cours
                        <span class="badge bg-info text-dark ms-2"><?= count($cours) ?> cours</span>
                    </h5>
                    <div class="d-flex gap-2 align-items-center flex-wrap">
                        <div class="input-group input-group-sm" style="max-width:200px;">
                            <span class="input-group-text bg-white"><i class="bi bi-search text-muted"></i></span>
                            <input type="text" id="searchCours" class="form-control border-start-0"
                                   placeholder="Rechercher…" oninput="searchTable('coursTable', this.value)">
                        </div>
                        <button class="btn btn-sm btn-outline-danger" onclick="exportTablePDF('coursTable', 'Cours', ['Titre','Description','Formateur'])">
            ...
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0" id="coursTable">
                        <thead>
                            <tr>
                                <th class="ps-4" style="cursor:pointer;" onclick="sortTableBy('coursTable',0,this)">
                                    Titre <i class="bi bi-arrow-down-up small text-muted"></i></th>
                                <th style="cursor:pointer;" onclick="sortTableBy('coursTable',1,this)">
                                    Description <i class="bi bi-arrow-down-up small text-muted"></i></th>
                                <th style="cursor:pointer;" onclick="sortTableBy('coursTable',2,this)">
                                    Formateur <i class="bi bi-arrow-down-up small text-muted"></i></th>
        ...
        <div class="card shadow mb-4">
            <div class="card-header border-bottom">
                <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
                    <h5 class="m-0"><i class="bi bi-patch-check me-2 text-warning"></i>Mes Certificats
                        <span class="badge bg-warning text-dark ms-2"><?= count($certificats) ?> certificat<?= count($certificats) !== 1 ? 's' : '' ?></span>
                    </h5>
                    <div class="d-flex gap-2 align-items-center flex-wrap">
                        <div class="input-group input-group-sm" style="max-width:200px;">
                            <span class="input-group-text bg-white"><i class="bi bi-search text-muted"></i></span>
                            <input type="text" id="searchCertificats" class="form-control border-start-0"
                                   placeholder="Rechercher…" oninput="searchTable('certificatsTable', this.value)">
                        </div>
                        <button class="btn btn-sm btn-outline-danger" onclick="exportTablePDF('certificatsTable', 'Certificats', ['Nom','Cours Lié','Organisation','Date'])">
                            <i class="bi bi-file-earmark-pdf me-1"></i>PDF
                        </button>
                    </div>
                </div>
            ...
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0" id="certificatsTable">
                        <thead>
                            <tr>
                                <th class="ps-4" style="cursor:pointer;" onclick="sortTableBy('certificatsTable',0,this)">
                                    Nom <i class="bi bi-arrow-down-up small text-muted"></i></th>
                                <th style="cursor:pointer;" onclick="sortTableBy('certificatsTable',1,this)">
                                    Cours Lié <i class="bi bi-arrow-down-up small text-muted"></i></th>
                                <th style="cursor:pointer;" onclick="sortTableBy('certificatsTable',2,this)">
                                    Organisation <i class="bi bi-arrow-down-up small text-muted"></i></th>
                                <th style="cursor:pointer;" onclick="sortTableBy('certificatsTable',3,this)">
                                    Date <i class="bi bi-arrow-down-up small text-muted"></i></th>
                                <th>Fichier</th>
                            </tr>
                        </thead>
                        <tbody>
...
<div class="modal fade" id="quizDemandeModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <form action="/INTEG/certifications/demanderCertification" method="POST" enctype="multipart/form-data">
                <div class="modal-body">
...
<div class="modal fade" id="passerQuizModal" tabindex="-1" data-bs-backdrop="static" data-bs-keyboard="false">
    <div class="modal-dialog modal-xl modal-dialog-scrollable">
        <div class="modal-content">
            ...
            <form action="/INTEG/certifications/passerQuiz" method="POST" id="quizForm">
...
// ── Recherche universelle (colonnes de données seulement) ────────
const _searchCols = {
    'coursTable':      [0, 1, 2],
    'certificatsTable':[0, 1, 2, 3],
};
...
function sortTableBy(tableId, colIndex, thEl) {
    ...
    // Reset tous les icônes du tableau
    document.querySelectorAll('#' + tableId + ' thead th i').forEach(i => {
        i.className = 'bi bi-arrow-down-up small text-muted';
    });
    if (thEl) {
        const icon = thEl.querySelector('i');
        if (icon) icon.className = asc ? 'bi bi-arrow-up small text-info' : 'bi bi-arrow-down small text-info';
    }

    const tbody = document.querySelector('#' + tableId + ' tbody');
    if (!tbody) return;
    const rows = Array.from(tbody.querySelectorAll('tr')).filter(r => r.style.display !== 'none');
    const hidden = Array.from(tbody.querySelectorAll('tr')).filter(r => r.style.display === 'none');
