...
class DemandeCertification extends Model
{
    ...
    {
        $stmt = $this->db->prepare("SELECT * FROM demandes_certification ORDER BY soumise_le DESC");
        $stmt->execute();
    ...
    {
        $stmt = $this->db->prepare("SELECT * FROM demandes_certification WHERE statut = ? ORDER BY soumise_le DESC");
        $stmt->execute([$statut]);
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
    ...
    {
        $stmt = $this->db->prepare("SELECT COUNT(*) FROM demandes_certification WHERE statut = ?");
        $stmt->execute([$statut]);
    ...
    {
        $stmt = $this->db->prepare("UPDATE demandes_certification SET statut = 'quiz_envoye' WHERE id = ?");
        try {
    ...
    {
        $stmt = $this->db->prepare("SELECT * FROM demandes_certification WHERE id = ?");
        $stmt->execute([$id]);
